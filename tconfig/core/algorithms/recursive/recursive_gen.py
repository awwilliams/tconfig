...
class RecursiveGenerator(Generator):
    ...
    def determine_square_setup(self):
        if self.num_parms <= 3:
            ...
            self.square_size = utils.find_next_prime_power(self.second_max_num_values)
            max_parms = self.square_size + 1
            self.num_rounds = math.ceil(math.log(self.num_parms, max_parms))
            if self.num_rounds == 2:
                if self.num_parms > max_parms * (max_parms - 1):
                    num_configs_v1 = 2 * self.square_size ** self.coverage_degree - 1
                else:
                    num_configs_v1 = (
                        2 * self.square_size ** self.coverage_degree
                        - 1
                        - self.square_size
                    )
    ...
    def determine_round_groupings(self):
        if self.num_rounds > 0:
            ...
            self.extra_parms_repeat_factor = [[0] * self.num_rounds] * self.num_rounds

            self.num_configs_per_round.append(self.square_size ** self.coverage_degree)
            ...
            while capacity < self.num_parms:
                ...
                self.num_configs_per_round[current_round] = (
                    self.square_size ** self.coverage_degree - 1
                )
                current_round += 1
                capacity = self.find_capacity()
            ...
            if self.square_size < self.max_num_values:
                num_configs += (
                    self.max_num_values - self.square_size
                ) * self.second_max_num_values

        basic_capacity = (
            self.group_repeat_factor[self.num_rounds - 1]
        ...
        for round_index in range(1, self.num_rounds):
            if self.round_group_size[round_index] == self.square_size + 1:
                ...
                self.extra_parms_per_round.append(
                    basic_capacity
                    // (
                        self.group_repeat_factor[round_index]
                        * self.round_group_size[round_index]
                    )
                )
            if round_index + 1 < self.num_rounds:
                ...
                for sub_seq_index in range(round_index + 2, self.num_rounds):
                    self.extra_parms_repeat_factor[round_index][sub_seq_index] = (
                        self.extra_parms_repeat_factor[round_index][sub_seq_index - 1]
                        * self.round_group_size[sub_seq_index]
                    )
    ...
    def build_configurations(self) -> np.ndarray:
        ...
        for round_num in range(0, self.num_rounds):
            ...
            repeat_factor = basic_capacity // (
                self.group_repeat_factor[round_num] * self.round_group_size[round_num]
            )
            if round_num == 0:
            ...
            for prior_round in range(1, round_num):
                ...
                round_grid = np.hstack([round_grid, np.tile(add_grid, reps=num_groups)])
        ...
        over_sized_grid = oa_gen.oversized_parameter(
            self.max_num_values,
        ...
        config_grid = self.adjust_values(
            final_grid[:, offset:], self.square_size - self.second_max_num_values + 1
        )
