...
class GeneratorRequestSchema(Schema):
    ...
    def validate_schema(self, data, **kwargs):
        ...
        if coverage_degree < 1:
            raise ValidationError(
                f"Invalid coverage degree '{coverage_degree}': must be > 1"
            )
        ...
        if algorithm_name == "recursive" and coverage_degree >= 3:
            raise ValidationError(
                f"Coverage degree higher than 2 not yet implemented for recursive algorithm"
            )
    ...
    def make_gen_request(self, data, **kwargs):
        ...
        if orig_df.empty:
            ...
            data["existing_configurations"] = ConfigurationSet(
                data_frame=orig_df[parameter_names]
            )
